...
export const HistoryPage: React.FC = () => {
  ...
  const fetchAnalyses = async () => {
    setLoading(true);
    try {
      const res = await api.getAnalyses(100, 0, riskFilter || undefined, classFilter || undefined);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-[#0d1626] to-slate-900 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-xl">
            Complete immutable ledger of voice anti-spoofing analyses, probability scores, and evidence reasoning.
          </p>
        </div>
      ...
      <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800">
        {loading ? (
          <div className="p-12 text-center text-xs font-mono text-cyan-400">Loading historical forensic logs...</div>
        ) : filtered.length === 0 ? (
                ...
                {filtered.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="p-3 font-mono text-slate-400">
                      {a.timestamp ? a.timestamp.replace('T', ' ').substring(0, 19) : '-'}
                    </td>
                    ...
                    <td className="p-3">
                      <RiskBadge classification={a.prediction?.classification} />
                    </td>
                    <td className="p-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="w-full max-w-xl p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-start justify-between pb-3 border-b border-slate-800">
              <div>
                <h3 className="text-lg font-bold text-white font-display">Forensic Analysis #{selectedAnalysis.id}</h3>
                <span className="text-xs font-mono text-slate-400">{selectedAnalysis.timestamp}</span>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                <span className="text-slate-400 block mb-1">AI Voice Synthetic Prob:</span>
                <span className="text-xl font-bold font-mono text-rose-400">{((selectedAnalysis.prediction.ai_probability || 0) * 100).toFixed(1)}%</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                <span className="text-slate-400 block mb-1">Risk Score / Level:</span>
                <RiskBadge level={selectedAnalysis.risk.level} score={selectedAnalysis.risk.score} />
              </div>
            </div>

            {selectedAnalysis.explanation.length > 0 && (
              <div className="space-y-1.5">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300">Evidence Points:</h4>
            ...
            <div className="p-3 rounded-xl bg-slate-950/70 border border-amber-500/30 text-xs">
              <span className="font-bold text-amber-300 block mb-1">Recommended Action:</span>
              <p className="text-slate-300">{selectedAnalysis.risk.recommended_action}</p>
            </div>
          </div>
        </div>
      )}
