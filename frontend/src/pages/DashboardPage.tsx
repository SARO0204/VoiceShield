      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-[#0d1627] to-slate-900 border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-xl">
            Real-time deepfake acoustic inspection (AASIST), social engineering
            heuristic scoring, and fraud interception.
          </p>
        </div>
        ...
        {kpis.map((kpi, idx) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <span
                  className={`text-2xl font-black font-mono-numbers ${kpi.color}`}
                >
                  {kpi.value}
                </span>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-mono text-slate-400">
              Total: {data.total_calls_analyzed}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-800">
            {data.risk_distribution.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-1.5 rounded-lg bg-slate-950/40"
              >
          ...
          <div className="space-y-2.5 my-auto">
            <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 text-xs space-y-1">
              ...
              <p className="text-[11px] text-slate-400">
                Continuous 2-4 second sliding window inference with sub-100ms
                response.
              </p>
            </div>
            ...
            <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 text-xs space-y-1">
              ...
              <p className="text-[11px] text-slate-400">
                Detects urgent wire transfers, OTP requests, and impersonation
                claims.
              </p>
            </div>
          </div>
      ...
      <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <span className="text-xs text-slate-400">
              Dynamic log of analyzed speech audio
            </span>
          </div>
        ...
        {data.recent_calls.length === 0 ? (
          <div className="p-8 text-center rounded-xl bg-slate-950/40 border border-slate-800 text-slate-400 text-xs font-mono">
            No voice analyses recorded yet. Use 'Analyze Audio' or 'Live
            Protection' to run the first detection.
          </div>
        ) : (
                ...
                {data.recent_calls.map((call) => (
                  ...
                  >
                    <td className="p-3 font-mono text-slate-400">
                      {call.started_at
                        ? call.started_at.split("T")[1]?.substring(0, 8)
                        : "--:--:--"}
                    </td>
